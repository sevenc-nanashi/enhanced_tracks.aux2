...
local function standard_interpolation(ratio, p0, p1, p2, p3)
  ...
  for axis_index = 1, #p1 do
    local previous_control = p1[axis_index]
      + (previous_length * (p2[axis_index] - p1[axis_index]) + current_length * (p1[axis_index] - p0[axis_index]))
        / previous_total
        / 3.0
    local next_control = p2[axis_index]
      - (current_length * (p3[axis_index] - p2[axis_index]) + next_length * (p2[axis_index] - p1[axis_index]))
        / next_total
        / 3.0
